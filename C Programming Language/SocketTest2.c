...
{
    int connfd[MAX_CLIENT] = {0}, n = 0, listenfd, client = 0;
    ...
    while (1)
    {
        connfd[client++] = accept(listenfd, (struct sockaddr*)NULL, NULL);
        ...
        for (int i = 0; i; i++)
        {
			while((n = read(connfd[i], recvBuff, sizeof(recvBuff)-1)) > 0)
			{
			   recvBuff[n] = 0;
